...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Received record for agent adikart form PDF:', record);
    console.log('Image data received for agent adikart form:', !!imageData);

    // Use the agent adikart form template
    const templatePath = path.join(process.cwd(), 'public', 'pdf', 'agent', 'agent_adikart_form.pdf');

    if (!fs.existsSync(templatePath)) {
      throw new Error(`Agent adikart form template not found: ${templatePath}`);
    }

    console.log('Using agent adikart form template:', templatePath);
    ...
    // Base field mappings for the agent adikart form - adjust coordinates based on the actual form
    const baseFieldMappings = [
      ...
      { field: 'tehsil', x: 95, y: 380, label: 'Tehsil' },
      { field: 'district', x: 85, y: 415, label: 'District' },
      { field: 'mobile', x: 315, y: 415, label: 'Mobile' },    
      { field: 'workArea', x: 210, y: 452, label: 'Work Area' },
    ...
    const filename = `agent_adikart_form_${safeName}.pdf`;
    ...
    return NextResponse.json(
      {
        error: 'Failed to generate agent adikart form PDF',
