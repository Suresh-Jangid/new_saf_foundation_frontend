...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Received disability cycle data:', data);
    console.log('Image data received:', !!imageData);

    // Use the disability cycle template
    const templatePath = path.join(process.cwd(), 'public', 'pdf', 'disablity_cycle', 'disablity_cycle.pdf');

    if (!fs.existsSync(templatePath)) {
      throw new Error(`Disability cycle template PDF not found: ${templatePath}`);
    }

    console.log('Using disability cycle template:', templatePath);
    ...
    // Base field mappings for disability cycle form
    ...
    const baseFieldMappings: { field: keyof typeof data; x: number; y: number }[] = [
      // Header section
      { field: 'formNumber' as any, x: 110, y: 240 },   // Form number (top left box)
      { field: 'applicationDate' as any, x: 480, y: 245 }, // Application date
    ...
    const anchorTargetFor: Record<string, { x: number; y: number }> = {
      'formNumber': { x: 110, y: 240 },
      'applicationDate': { x: 480, y: 245 },
    };
    ...
    const filename = `disability_cycle_${formNumber}.pdf`;
    ...
    console.error('Error generating disability cycle PDF:', error);
    return NextResponse.json(
      {
        error: 'Failed to generate disability cycle PDF',
