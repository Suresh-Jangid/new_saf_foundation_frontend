...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Received sewing machine data:', data);
    console.log('Image data received:', !!imageData);

    // Use the girl sewing machine template
    const templatePath = path.join(process.cwd(), 'public', 'pdf', 'marriage_application', 'girl_sewing_machine.pdf');

    if (!fs.existsSync(templatePath)) {
      throw new Error(`Sewing machine template PDF not found: ${templatePath}`);
    }

    console.log('Using sewing machine template:', templatePath);
    ...
    // Base field mappings for sewing machine form
    ...
    const baseFieldMappings: { field: keyof typeof data; x: number; y: number }[] = [
      // Header section
      { field: 'marriageNumber' as any, x: 110, y: 235 },   // Marriage number (top left box)
      { field: 'applicationDate' as any, x: 450, y: 235 }, // Application date

      // Main form section - Adjusted based on form structure
      { field: 'applicantName' as any, x: 90, y: 350 },    // Applicant name
      { field: 'fatherName' as any, x: 305, y: 350 },      // Father's name
      { field: 'motherName' as any, x: 110, y: 385 },      // Mother's name
      { field: 'dateOfBirth' as any, x: 350, y: 385 },     // Date of birth
      { field: 'gotra' as any, x: 80, y: 425 },            // Gotra
    ...
    const filename = `sewing_machine_distribution_${marriageNumber}.pdf`;
    ...
    console.error('Error generating sewing machine PDF:', error);
    return NextResponse.json(
      {
        error: 'Failed to generate sewing machine PDF',
