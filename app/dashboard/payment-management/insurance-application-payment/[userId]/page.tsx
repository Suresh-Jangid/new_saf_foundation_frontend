...
export default function AddPaymentPage() {
  ...
  // Fetch user details and payments from API (Insurance)
  useEffect(() => {
    if (!userId) return;
    let mounted = true;
    ...
    (async () => {
      try {
        ...
        const [userRes, installmentsRes] = await Promise.all([
          APIService.getInsuranceApplicationById(String(userId)),
          APIService.getInsuranceApplicationInstallments(String(userId)),
        ]);

        if (!mounted) return;

        const fetchedUser = Array.isArray(userRes.data)
          ? userRes.data[0]
          : userRes.data || null;
        ...
        console.error('Failed to load insurance user/payments', err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleAddPayment = async (razorpayData?: any) => {
    ...
    try {
      ...
      const payload = {
        application_insurance_id: user?.id || userId,
      ...
      await APIService.addInsuranceApplicationInstallment(payload as any);
      const updatedUserRes = await APIService.getInsuranceApplicationById(String(userId));

      // Update user details with fresh data
      const updatedUser = Array.isArray(updatedUserRes.data)
        ? updatedUserRes.data[0]
        : updatedUserRes.data || null;
      setUser(updatedUser || null);

      // Fetch updated payments from API
      const updatedInstallmentsRes = await APIService.getInsuranceApplicationInstallments(String(userId));
      ...
      console.error("Failed to add insurance installment", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-4 w-full">
      <div className="flex items-center mb-6 gap-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          Add Payment Details for Insurance Bima Application
          <span className="block text-base font-normal text-gray-700">
            बीमा आवेदन के लिए भुगतान विवरण जोड़ें
          </span>
        </h1>
      </div>
                ...
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-md">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <RazorpayPayment
                      ...
                      description={`Insurance Bima Application Payment - ${user?.applicantName || 'Applicant'}`}
