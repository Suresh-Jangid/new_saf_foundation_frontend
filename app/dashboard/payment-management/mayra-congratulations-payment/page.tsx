...
export default function MayraCongratulationsPaymentListPage() {
  const [applications, setApplications] = useState<MayraCongratulationsData[]>([]);
  ...
  const fetchApplications = async (filters?: ApplicationFilters) => {
    try {
      ...
      const response = await APIService.getMayraCongratulations(filtersWithUserInfo);
  ...
  const handleGenerateReceipt = async (application: MayraCongratulationsData) => {
    try {
      setGeneratingReceipt(application.id);
      ...
      a.href = url;
      a.download = `Mayra_Receipt_${application.mayraNumber}.pdf`;
    ...
    <div className="p-3 md:p-6 relative">
      <div className="mb-4 md:mb-6">
        <h1 className="text-lg md:text-2xl font-bold text-gray-900">
          Mayra Congratulations Payment Distribution
        </h1>
        <p className="text-xs md:text-sm text-gray-600">
          Manage and review all mayra congratulations application records.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="relative w-full">
          ...
          <Input
            placeholder="Search by name, mayra number, or code..."
            value={searchTerm}
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm md:text-lg">
            All Mayra Congratulations Payment Records ({applications.length} total)
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            <div className="text-center py-8 text-gray-500 px-4">
              Loading mayra congratulations applications...
            </div>
          ) : paginatedApplications.length === 0 ? (
            <div className="text-center py-8 text-gray-500 px-4">
              {hasActiveFilters
                ? "No mayra congratulations applications found matching your search."
                : "No mayra congratulations applications found."}
            </div>
          ) : (
            <>
              {/* Mobile Card View */}
                  ...
                  <div key={application.id} className="border rounded-lg p-4 space-y-3">
                    ...
                    {application.payment_status === 1 ? (
                      <Button
                        ...
                        onClick={() => handleGenerateReceipt(application)}
                        disabled={generatingReceipt === application.id}
                      >
                        {generatingReceipt === application.id ? "Generating..." : "Generate Receipt"}
                      </Button>
                    ) : (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() =>
                          router.push(`/dashboard/payment-management/mayra-congratulations-payment/${application.id}`)
                        }
                        className="w-full"
                      >
                        Add Payment
              ...
              <div className="hidden md:block">
                <div className="w-full overflow-x-auto">
                  <Table className="w-full min-w-[900px]">
                    <TableHeader>
                      <TableRow>
                        <TableHead className="text-xs">Mayra Number</TableHead>
                    ...
                    <TableBody>
                      {paginatedApplications.map((application) => (
                        <TableRow key={application.id}>
                          <TableCell className="font-medium text-sm">{application.mayraNumber}</TableCell>
