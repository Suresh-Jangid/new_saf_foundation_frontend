...
import {
  ...
  ShubhLaxmiInstallment,
} from "@/lib/shubh-laxmi-service";
import { formatDate } from "@/lib/utils";

export default function ShubhLaxmiDetailsPage() {
  const params = useParams();
  const router = useRouter();
  const id = String(params?.id || "");

  const [record, setRecord] = useState<ShubhLaxmiRegistration | null>(null);
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const res = await ShubhLaxmiService.getRegistrationById(id);
      if (res && res.data) {
        setRecord(res.data);
      }
    } catch (err: any) {
  ...
  const handleAddInstallment = async (e: React.FormEvent) => {
    ...
    try {
      await ShubhLaxmiService.addInstallment(record.id, {
        amount: 300,
    ...
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center gap-2">
        ...
        <span className="text-sm text-muted-foreground">
          शुभलक्ष्मी विवरण लोड हो रहा है... / Loading details...
        </span>
      </div>
    );
  }
    ...
    return (
      <div className="p-6 max-w-xl mx-auto text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          अनुरोधित शुभलक्ष्मी रिकॉर्ड उपलब्ध नहीं है या हटा दिया गया है।
        </p>
        <Button onClick={() => router.push("/dashboard/shubh-laxmi")} variant="outline">
  ...
  const installments: ShubhLaxmiInstallment[] = record.installments || [];
  const totPaid =
  ...
  const totPending =
    record.pendingAmount !== undefined
      ...
      : Math.max(0, (record.totalAmount || 3100) - totPaid);
  ...
  return (
    <RoleGuard requiredModule="shubh_laxmi" requiredAction="view">
      <div className="space-y-6 p-4 md:p-6 pb-16 max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-mono text-base font-bold text-yellow-600">
                  {record.formNumber}
                </span>
                <Badge variant="outline" className="bg-yellow-500/10 text-yellow-700 dark:text-yellow-300 border-yellow-300">
                  SHUBH_LAXMI
                </Badge>
                <Badge variant="outline" className="bg-yellow-500/10 text-yellow-700 dark:text-yellow-300 border-yellow-300">
                  UNIFIED_POOL
                ...
                >
                  {record.gender === "Female" ? "महिला (Female)" : "पुरुष (Male)"}
                </Badge>
                {record.epinCode && (
        ...
        {/* 12-Month Benefit & Lifecycle Notice */}
        <Card className="border-yellow-200 dark:border-yellow-900/40 bg-yellow-50/30 dark:bg-yellow-950/20 shadow-sm">
          <CardContent className="p-4 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-yellow-600" />
              <div>
                <strong>12 माह लाभ स्थिति:</strong> 12 माह पूर्ण होने पर भुगतान सहायता/लाभ उपलब्ध होगा (20% नियमानुसार कटौती लागू)।
              </div>
            </div>
            {record.missedInstallmentsCount && record.missedInstallmentsCount >= 3 ? (
              <Badge variant="destructive" className="flex items-center gap-1">
                <AlertTriangle className="h-3 w-3" /> 3 किश्तें छूटी / Lifecycle Warning
              </Badge>
            ) : (
              <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-300">
                नियमित खाता / Active Account
              </Badge>
            )}
          </CardContent>
        </Card>
          ...
          <Card className="border-border/60 shadow-sm">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="border-t border-border/60 pt-4">
                <span className="text-xs font-semibold text-yellow-700 dark:text-yellow-300 flex items-center gap-1.5 mb-2">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div>
                ...
                <CardDescription className="text-xs text-muted-foreground mt-0.5">
                  शुभलक्ष्मी ₹300 निर्धारित किश्त भुगतान रिकॉर्ड / Payment logs
                </CardDescription>
              </div>
              <Button
                size="sm"
          ...
          <CardContent className="p-6">
            {installments.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground text-xs space-y-2">
                ...
                <div>शुभलक्ष्मी खाते में अभी तक कोई किश्त दर्ज नहीं की गई है।</div>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-border/60">
                    {installments.map((inst, idx) => (
                      <tr key={inst.id || idx} className="hover:bg-muted/30">
                        ...
                        <td className="px-4 py-3 text-xs font-bold text-yellow-600 dark:text-yellow-400">
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <form onSubmit={handleAddInstallment} className="space-y-4 pt-2">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="shubhDetailInstAmount" className="text-xs font-semibold">
                  किश्त राशि / Amount (₹) <span className="text-destructive">*</span>
                </Label>
                <div className="relative">
                  <Input
                    id="shubhDetailInstAmount"
                    type="number"
                    value={300}
                    readOnly
              ...
              {/* Payment Date */}
              <div className="space-y-1.5">
                <Label htmlFor="shubhDetailInstDate" className="text-xs font-semibold">
                  भुगतान दिनांक / Payment Date <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="shubhDetailInstDate"
                  type="date"
              ...
              {/* Payment Mode */}
              <div className="space-y-1.5">
                <Label htmlFor="shubhDetailInstMode" className="text-xs font-semibold">
                  भुगतान माध्यम / Payment Mode <span className="text-destructive">*</span>
                </Label>
                <select
                  id="shubhDetailInstMode"
              ...
              {/* Rashid Number */}
              <div className="space-y-1.5">
                <Label htmlFor="shubhDetailRashidNo" className="text-xs font-semibold">
                  रसीद संख्या / Receipt / Rashid Number (Optional)
                </Label>
                <Input
                  id="shubhDetailRashidNo"
              ...
              {/* Note */}
              <div className="space-y-1.5">
                <Label htmlFor="shubhDetailInstNote" className="text-xs font-semibold">
                  टिप्पणी / Remarks (Optional)
                </Label>
                <Input
                  id="shubhDetailInstNote"
